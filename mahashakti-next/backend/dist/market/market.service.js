...
let MarketService = MarketService_1 = class MarketService {
    constructor(angelOneService, symbolMasterService) {
    ...
    async getBigMovers(minChange = 15, maxChange = 20) {
        try {
            const bigMovers = [
                {
                    symbol: 'ADANIPORTS',
            ...
            return bigMovers.filter((stock) => Math.abs(stock.changePercent) >= minChange && Math.abs(stock.changePercent) <= maxChange);
        }
        catch (error) {
    ...
    async searchSymbols(query) {
        ...
        return allSymbols.filter((s) => s.symbol.toLowerCase().includes(query.toLowerCase()) ||
...
exports.MarketService = MarketService = MarketService_1 = __decorate([
    ...
    __metadata("design:paramtypes", [angel_one_service_1.AngelOneService,
        symbol_master_service_1.SymbolMasterService])
], MarketService);
